...
{
    private static readonly ProtoId<JobPrototype> Passenger = "StellarJobServiceAssistant"; // Stellar jobs instead of wizden jobs
    private static readonly ProtoId<JobPrototype> Engineer = "StellarJobEngineeringEngineer"; // Stellar jobs instead of wizden jobs
    private static readonly ProtoId<JobPrototype> Captain = "StellarJobCommandCaptain"; // Stellar jobs instead of wizden jobs
...
";
    ...
    public async Task JobPriorityTest()
    {
        ...
        var ticker = pair.Server.System<GameTicker>();
        Assert.That(ticker.RunLevel, Is.EqualTo(GameRunLevel.PreRoundLobby));
        // Assert.That(pair.Client.AttachedEntity, Is.Null); // ES
