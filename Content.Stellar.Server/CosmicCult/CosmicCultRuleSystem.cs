...
public sealed class CosmicCultRuleSystem : GameRuleSystem<CosmicCultRuleComponent>
{
    ...
    [Dependency] private readonly GibbingSystem _body = default!;
    ...
    private readonly SoundSpecifier _briefingSound = new SoundPathSpecifier("/Audio/_ST/CosmicCult/antag_cosmic_briefing.ogg");
    private readonly SoundSpecifier _deconvertSound = new SoundPathSpecifier("/Audio/_ST/CosmicCult/antag_cosmic_deconvert.ogg");
    ...
    public override void Initialize()
    {
        ...
        Subs.CVar(_config,
            STCCVars.CosmicCultT2RevealDelaySeconds,
            value => _t2RevealDelay = TimeSpan.FromSeconds(value),
            true);
        Subs.CVar(_config,
            STCCVars.CosmicCultT3RevealDelaySeconds,
            value => _t3RevealDelay = TimeSpan.FromSeconds(value),
            true);
        Subs.CVar(_config,
            STCCVars.CosmicCultFinaleDelaySeconds,
            value => _finaleDelay = TimeSpan.FromSeconds(value),
            true);
        Subs.CVar(_config,
            STCCVars.CosmicCultStewardVoteTimer,
            value => _voteTimer = TimeSpan.FromSeconds(value),
            true);
        Subs.CVar(_config,
            STCCVars.CosmicCultStewardVoteDelayTimer,
    ...
    private void EndRoundVoid(EntityUid player, EntityCoordinates spawnTgt, CosmicCultRuleComponent cultRule)
    {
        ...
        _body.Gib(player); // you don't need that body anymore
    }
